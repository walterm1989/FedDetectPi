...
# -*- coding: utf-8 -*-
"""
05_keypoints_local_vs_flower.py
-------------------------------
Comparativa de KeyPoints entre LOCAL y FLOWER.
Lee el consolidado y genera tablas + gráficos para method=KeyPoints
en los dos scopes: local vs flower. Incluye %slow_frames usando el P95
de latencia del grupo LOCAL como umbral de referencia.

Uso:
  python Metricas_informe/Graficos/05_keypoints_local_vs_flower/05_keypoints_local_vs_flower.py \
      --in Metricas_informe/raw/metrica_consolidada.csv \
      --out-dir Metricas_informe/Graficos/05_keypoints_local_vs_flower
"""
...
def plot_box_latency(df: pd.DataFrame, out_file: Path):
    # Boxplot latencia para KP-Local y KP-Flower
    labels, data = [], []
    for s, g in df.groupby("scope"):
        label = "KP-Local" if str(s).lower()=="local" else "KP-Flower"
        labels.append(label)
    ...
    plt.title("Distribución de latencia (ms) - KeyPoints")
...
def main():
    parser = argparse.ArgumentParser(description="KeyPoints: comparativa Local vs Flower con %slow_frames.")
    # Alias extra para PowerShell
    ...
    # Filtro KeyPoints y scopes de interés
    for col in ["fps","latency_ms","cpu_percent","ram_mb"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df[(df["method"]=="KeyPoints") & (df["scope"].str.lower().isin(["local","flower"]))]
    ...
    if df.empty:
        raise ValueError("No hay datos para KeyPoints en scopes local/flower.")
    ...
    # Guardar CSVs
    out_csv = out_dir / "keypoints_local_vs_flower.csv"
    summary.to_csv(out_csv, index=False)
    deltas.to_csv(out_dir / "keypoints_local_vs_flower_deltas.csv", index=False)

    # ---- Gráficos ----
    plot_df = summary.copy()
    plot_df["label"] = plot_df["scope"].map(lambda s: "KP-Local" if s=="local" else "KP-Flower")

    plot_bar(plot_df, "label", "fps_mean", "FPS medio - KeyPoints (Local vs Flower)", out_dir / "kp_fps_mean.png")
    plot_bar(plot_df, "label", "latency_ms_mean", "Latencia media (ms) - KeyPoints", out_dir / "kp_latency_mean.png")
    plot_bar(plot_df, "label", "efficiency_fps_per_10cpu", "Eficiencia (FPS por 10% CPU) - KeyPoints", out_dir / "kp_efficiency.png")
    plot_bar(plot_df, "label", "slow_frames_vs_localP95_%", "% frames lentos vs P95 Local - KeyPoints", out_dir / "kp_slow_frames.png")
    plot_box_latency(df, out_dir / "kp_latency_box.png")

    # Resumen Markdown
    summary_md = out_dir / "summary_kp_local_vs_flower.md"
    with open(summary_md, "w", encoding="utf-8") as f:
        f.write("# KeyPoints: Local vs Flower\n\n")
        f.write(f"- Dataset: {in_csv}\n")
        f.write(f"- Filas totales (KP local+flower): {len(df)}\n")
