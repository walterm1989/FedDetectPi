...
# -*- coding: utf-8 -*-
"""
02_comparativa_local.py
-----------------------
Comparativa en LOCAL entre BoundingBoxes y KeyPoints.
Lee el consolidado y genera tablas + gráficos para ambas metodologías solo con scope=local.

Uso:
  python Metricas_informe/report/02_comparativa_local.py \
...
def plot_box_latency(df: pd.DataFrame, out_file: Path):
    # Boxplot de latencia para BB-Local y KP-Local
    labels = []
    data = []
    for m, g in df.groupby("method"):
        labels.append("BB-Local" if "BoundingBoxes" in str(m) else "KP-Local")
        data.append(pd.to_numeric(g["latency_ms"], errors="coerce").dropna().values)
    if not data:
        return
    plt.figure()
    plt.boxplot(data, labels=labels, showfliers=False)
    plt.title("Distribución de latencia (ms) - Local")
...
def main():
    parser = argparse.ArgumentParser(description="Comparativa LOCAL: BoundingBoxes vs KeyPoints.")
    ...
    # Filtro LOCAL y solo métodos de interés
    for col in ["fps","latency_ms","cpu_percent","ram_mb"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df[(df["scope"].str.lower()=="local") & (df["method"].isin(["BoundingBoxes","KeyPoints"]))]
    ...
    if df.empty:
        raise ValueError("No hay datos para scope=local y métodos BoundingBoxes/KeyPoints.")
    ...
    # Guardar CSVs
    out_csv = out_dir / "local_comparison.csv"
    summary.to_csv(out_csv, index=False)
    deltas.to_csv(out_dir / "local_comparison_deltas.csv", index=False)

    # Gráficos (barras y boxplot)
    plot_df = summary.copy()
    plot_df["label"] = plot_df["method"].map(lambda m: "BB-Local" if "BoundingBoxes" in str(m) else "KP-Local")

    plot_bar(plot_df, "label", "fps_mean", "FPS medio - Local", out_dir / "local_fps_mean.png")
    plot_bar(plot_df, "label", "latency_ms_mean", "Latencia media (ms) - Local", out_dir / "local_latency_mean.png")
    plot_bar(plot_df, "label", "efficiency_fps_per_10cpu", "Eficiencia (FPS por 10% CPU) - Local", out_dir / "local_efficiency.png")
    plot_box_latency(df, out_dir / "local_latency_box.png")

    # Resumen Markdown
    summary_md = out_dir / "summary_local.md"
    with open(summary_md, "w", encoding="utf-8") as f:
        f.write("# Comparativa LOCAL (BoundingBoxes vs KeyPoints)\n\n")
        f.write(f"- Dataset: {in_csv}\n")
        f.write(f"- Filas totales (LOCAL): {len(df)}\n\n")
